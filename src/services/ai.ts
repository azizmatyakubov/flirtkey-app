...
import { Girl, Culture, AnalysisResult } from '../types';
...
Keep ${stage.tone} tone. Be specific and actionable.`;
  ...
  // Fallback
  return {
    suggestions: [{ type: 'safe', text: 'Sorry, try again!', reason: 'Error occurred' }],
...
export async function analyzeScreenshot(
  ...
  imageBase64: string,
  girl: Girl | null,
  _userCulture: Culture // Prefixed with underscore to indicate intentionally unused
): Promise<AnalysisResult> {
  const response = await axios.post(
    'https://api.openai.com/v1/chat/completions',
    {
